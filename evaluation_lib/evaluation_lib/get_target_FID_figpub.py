...
if __name__ == "__main__":
    ...
    args_json = load_json(json_file='./config/celeba/facescrub_FaceNet64.json')
    ...
    for data_ in dataset:
        ...
        for _, (img_, label_) in enumerate(dataloader_test):
    ...
    print('label',iden,iden.shape)
    print('{}_target_fid.npy'.format(args_json["dataset"]["name"]))
    np.save('{}_target_fid.npy'.format(args_json["dataset"]["name"]),img300ids)
    np.save('{}_target_fid_label.npy'.format(args_json["dataset"]["name"]),iden)          
