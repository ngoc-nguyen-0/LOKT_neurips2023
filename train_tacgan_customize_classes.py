...
def get_wandb(is_wandb, project_id,project='tacgan',exp_dir='.'):
    global wandb_record
...
def get_args():
    ...
    parser.add_argument('--attackid', type=int, default=100)
...
def main():
    ...
    project_name = os.path.join('..','Results_label_only1',loaded_args_dataset['dataset']['d_priv'],loaded_args_dataset['dataset']['d_pub'],args.target_model,'tacgan_{}_wgan_{}'.format(args.alpha,args.attackid))
    ...
    num_classes=args.num_classes #just for saving
    ...
    _, test_loader = init_dataloader(loaded_args_dataset, loaded_args_dataset['dataset']['test_file_path'], args.batch_size, mode="test", attackid = args.attackid)
    ...
    gen,dis = loader.load_cgan_wgan(args,device)                                    
                #    '../Results_label_only/vggface2/ffhq/resnet50/tacgan_1.5_wgan_100/gen_1_iter_0001000.pth.tar',
                #    '../Results_label_only/vggface2/ffhq/resnet50/tacgan_1.5_wgan_100/dis_1_iter_0001000.pth.tar')
    print(dis)
    # gen,dis = loader.load_cgan_wgan(args,device)
    ...
    if args.is_wandb:
        project_id = "{}_{}_{}_{}_wgan{}".format(loaded_args_dataset['dataset']['d_priv'],loaded_args_dataset['dataset']['d_pub'],args.target_model,args.alpha,args.attackid)    
    ...
    n_uncond = 1000 # firstly, train G,D without loss_C 
    iter_t = 0
    iter_request = 0
    ...
    for n_iter in range(1, args.max_iteration + 1):
        ...
        for i in range(args.n_dis):  # args.ndis=5, Gen update 1 time, Dis update ndis times.
            if i == 0:
                #G is learnt to generate attacked id
            ...
            with torch.no_grad():                
                ...
                if n_iter > n_uncond:
                    fake = []                    
                    while len(fake)<args.batch_size:
                        fake_all, pseudo_y_all, _ = sample_from_gen(args, device, args.num_classes, gen,args.batch_size)
                        
                        iter_request +=1


                        fake_all = fake_all.detach()
                        y_fake_all = decision(fake_all, target_model)
                        # print('y_fake_all',y_fake_all)
                        # y_fake = y_fake.detach()    
                        index_y = y_fake_all < args.attackid   
                        if sum(index_y.shape)>0:
                            # print('index_y',sum(index_y))
                            if len(fake) ==0:
                                fake = fake_all[index_y]
                                pseudo_y = pseudo_y_all[index_y]
                                y_fake = y_fake_all[index_y]
                            else:
                                fake = torch.concat((fake,fake_all[index_y]),dim=0)
                                pseudo_y = torch.concat((pseudo_y,pseudo_y_all[index_y]),dim=0)
                                y_fake = torch.concat((y_fake,y_fake_all[index_y]),dim=0)

                            # print('fake',iter_t,fake.shape,len(fake),pseudo_y.shape,args.batch_size)
                        
                            ####### eval G
                            target_correct += y_fake_all.eq(pseudo_y_all.view_as(y_fake_all)).sum().item()
                        
                    fake = fake[:args.batch_size].detach()
                    pseudo_y = pseudo_y[:args.batch_size].detach()
                    y_fake = y_fake[:args.batch_size].detach()
                    # print('------------')
                else:
                    fake, pseudo_y, _ = sample_from_gen(args, device, args.num_classes, gen,args.batch_size)
                    fake = fake.detach()
